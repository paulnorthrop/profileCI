...
```{r, fig.alt = "Non-smooth plot of the profile log-likelihood for the parameter outcome2"}
plot(prof, parm = "outcome2")
```

To obtain smooth version of this plot, we call `profileCI()` with `mult = 8` and `faster = FALSE`, but this is a much slower calculation. 

```{r, fig.alt = "Smooth plot of the profile log-likelihood for the parameter outcome2"}
prof <- profileCI(glm.D93, loglik = poisson_loglik, mult = 8, faster = FALSE)
plot(prof, parm = "outcome2")
```

By default, once it has been determined that a limit lies between two values of the parameter, monotonic cubic spline interpolation is used to estimate the value of the limit. If a specific degree of accuracy is required then this can be set by passing a positive tolerance `epsilon` to the `itp` function in the [itp package](https://cran.r-project.org/package=itp).
